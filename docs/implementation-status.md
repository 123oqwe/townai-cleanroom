...
| Content, Channels, Billing, Operations | Implemented                          | package repositories and API tests; Content Library covers all verified document, draft, spreadsheet, deck, media, link, and session kinds | Provider-backed state is explicit when not configured                                                           |
| Routines and triggers                  | Implemented                          | `packages/routines`, immutable version history, expiring/revocable share grants, atomic cross-owner install, scheduler/trigger tests, UI   | A schedule only reports server-returned queued state                                                            |
