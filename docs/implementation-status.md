...
| Production API deployment              | Source-ready, infrastructure pending | `api/index.js`, `vercel.json`, `apps/api/src/index.ts` export the same built Vercel Function and worker endpoint | Requires real PostgreSQL, encryption key, worker secret, web origin, and project-level protection |
