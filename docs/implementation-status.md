...
| Production API deployment              | Source-ready, infrastructure pending | `api/index.ts`, `vercel.json`, `apps/api/src/index.ts` export the same Vercel Function and worker endpoint | Requires real PostgreSQL, encryption key, worker secret, web origin, and project-level protection |
