...
import { environment, type Environment } from "./config.js";
...
export interface RuntimeComposition {
  ...
  runtimeWorker: RuntimeWorker | undefined;
  routineScheduler: ReturnType<typeof createRoutineScheduler> | undefined;
  googleRoutinePoller: ReturnType<typeof createGoogleRoutinePoller>;
  googleCalendarPoller: ReturnType<typeof createGoogleCalendarPoller>;
  channelRepository: ReturnType<typeof createChannelRepository>;
...
export async function composeRuntime(
  ...
  // The API process never uses Codex; only the standalone worker process
  // may enable it. Vercel serverless is always "api".
  const harnessResponses = environment.RESPONSES_API_KEY !== undefined;
  const harnessCodex =
    role === "worker" &&
    environment.CODEX_EXEC_ENABLED &&
    codexBinaryResolvable(environment.CODEX_CLI_PATH);
  ...
  const harnessServerFactory = !(harnessResponses || harnessCodex)
    ? undefined
...
function createWorkerLoop(ctx: WorkerLoopContext): WorkerLoop | undefined {
  ...
  async function deliverChannels(): Promise<void> {
    await ctx.channelRepository.deliverNext({
      ...
      sendEmail: async (value) => {
        await ctx.googleApi.gmailSend(value);
      },
      resolveCredential: async ({ credentialRef }) => {
        const credential = ctx.channelCredentials[credentialRef];
  ...
  async function runOnce(): Promise<void> {
    if (ctx.routineScheduler !== undefined) await ctx.routineScheduler();
    await ctx.googleRoutinePoller.poll();
    await ctx.googleCalendarPoller.poll();
    ...
    if (today !== lastWikiUpkeepDate) {
      ...
      const ownerRows =
        await ctx.sql`select distinct owner_id from memories where status='active' union select distinct owner_id from wiki_documents where status='active' limit 500`;
      for (const row of ownerRows) {
        try {
          await ctx.knowledgeUpkeepScanner.scan({
            ownerId: asId<"user">(row["owner_id"]),
            staleAfterDays: 30,
          });
        } catch {
    ...
    if (ctx.runtimeWorker !== undefined)
      await ctx.runtimeWorker.runBatch(ctx.environment.WORKER_BATCH_SIZE);
    await deliverChannels();
    workerTimer = setTimeout(() => void runOnce(), 250);
    workerTimer.unref?.();
  }
