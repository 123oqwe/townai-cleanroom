...
export default function AccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleSaveCredential() {
    ...
    try {
      await client.accounts.updateCredential(id as ConnectedAccount["id"], {
        ...
        ...(scopes.trim() === "" ? {} : { scopes: scopes.split(",").map((s) => s.trim()).filter(Boolean) }),
      });
      setAccessToken("");
      setRefreshToken("");
      setScopes("");
  ...
  return (
    <section className="mx-auto max-w-2xl">
      <h1 className="mb-6 text-xl font-semibold tracking-tight">Account Details</h1>
      <div className="rounded-lg border p-4" style={{ background: "var(--panel)", borderColor: "var(--panel-border)" }}>
        ...
        <p className="text-xs" style={{ color: data.needsReauth ? "var(--danger)" : "var(--muted)" }}>
          {data.needsReauth ? "Needs reauth" : data.isActive ? "Active" : "Inactive"}
        </p>
        <div className="mt-2">
          ...
          <ul className="text-xs" style={{ color: "var(--muted)" }}>
            {Object.entries(data.capabilities).map(([key, val]) => (
              <li key={key}>{key}: {val ? "yes" : "no"}</li>
            ))}
          </ul>
        </div>
      </div>

      <div className="mt-4 rounded-lg border p-4" style={{ background: "var(--panel)", borderColor: "var(--panel-border)" }}>
