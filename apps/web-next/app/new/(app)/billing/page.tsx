        ...
        <>
          <div className="mb-6 rounded-lg border p-4" style={{ background: "var(--panel)", borderColor: "var(--panel-border)" }}>
            ...
            <p className="text-xs" style={{ color: "var(--muted)" }}>
              {data.billing?.creditBand} - {data.billing?.isBlocked ? "blocked" : "available"}
            </p>
            {data.period && (
          ...
          {data.usage === undefined || data.usage.length === 0 ? (
            <p className="text-sm" style={{ color: "var(--muted)" }}>No usage recorded for this period.</p>
          ) : (
            <ul className="flex flex-col gap-1">
              {data.usage.map((item) => (
                <li key={item.category} className="flex items-center justify-between rounded-md border p-2 text-sm" style={{ borderColor: "var(--panel-border)" }}>
