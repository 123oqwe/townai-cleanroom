...
export default function OperationsPage() {
  ...
  const { data: summaryData, error: sumError } = useSWR<
    OperationsSummary,
    TownApiError
  >(["operations-summary"], () => client.operations.summary());

  const {
    data: auditData,
    error: auditError,
    isLoading,
  } = useSWR<AuditPage, TownApiError>(["operations-audit", cursor], () =>
        ...
        <>
          ...
          {nextCursor !== null && (
            <button
              ...
              className="mt-4 w-full rounded-md border py-2 text-sm transition-colors hover:bg-[color:var(--background)]"
              style={{ borderColor: "var(--panel-border)" }}
            >
              Load more
            </button>
